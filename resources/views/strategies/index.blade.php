                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700">
                                    @foreach($strategies as $strategy)
                                        <tr>
                                            <td class="px-4 py-2 md:px-6 md:py-3">{{ $strategy->strategy_id }}</td>
                                            ...
                                            <td class="px-4 py-2 md:px-6 md:py-3">
                                                <a href="{{ route('strategies.edit', $strategy->strategy_id) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-600 mr-2">Edit</a>
                                                <form action="{{ route('strategies.destroy', $strategy->strategy_id) }}" method="POST" class="inline-block">
                                                    @csrf
                                                    @method('DELETE')
